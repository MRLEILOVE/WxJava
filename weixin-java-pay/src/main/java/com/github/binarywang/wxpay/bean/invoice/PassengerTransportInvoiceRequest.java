...
public class PassengerTransportInvoiceRequest implements Serializable {
  ...
  public static class BuyerInformation implements Serializable {
    ...
    /**
     * 购买方类型，必填。可选值：INDIVIDUAL（个人、其他）、ORGANIZATION（单位）。
     */
    private String type;
    ...
    /**
     * 购买方纳税人识别号，最长32个字符；购买方类型为 ORGANIZATION 时必填。
     */
    ...
    /**
     * 用户手机号。该字段为密文字段，调用方需使用微信支付公钥或微信支付平台证书公钥加密后传入。
     */
    private String phone;

    /**
     * 用户邮箱地址。该字段为密文字段，调用方需使用微信支付公钥或微信支付平台证书公钥加密后传入。
     */
    private String email;

    /**
     * 微信支付订单金额，仅在微信支付场景下有效，单位：分。
     */
    private Long amount;

    /**
     * 商户订单号，仅在微信支付场景下有效，最长32个字符。
     */
    @SerializedName("out_trade_no")
    private String outTradeNo;

    /**
     * 用户选择开具的发票类型。可选值：COMM_FAPIAO（增值税普通发票）、VAT_FAPIAO（增值税专用发票）。
     */
    @SerializedName("fapiao_bill_type")
    private String fapiaoBillType;

    /**
     * 用户申请开票时提交的留言信息，最长100个字符。
     */
